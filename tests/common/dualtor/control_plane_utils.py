...
class DBChecker:
    ...
    def _parse_intf_names(self):
        ...
        selected_intfs = set(
            _ for _ in mux_cable_table
            if mux_cable_table[_].get("cable_type", CableType.default_type) == self.cable_type
        )
        if self.intf_names == 'all':
            ...
            for intf in self.intf_names:
                if intf not in selected_intfs:
                    raise ValueError(
                        "Interface %s not in %s cable type" % (intf, self.cable_type))
    ...
    def _get_nbr_data(self, intf_name, dest_name):
        ...
        if ipaddress == "":
            logger.debug("Failed to fetch {}'s {} address in config db. ".format(
                intf_name, dest_name))
        ...
        logger.debug("Fetched neighbor entry data for {} {}: {}".format(
            intf_name, dest_name, nbr_data))
    ...
    def _get_mux_tunnel_route(self):
        ...
        tunnel_route = json.loads(self.duthost.shell(
            "show muxcable tunnel-route --json")['stdout'])
            ...
            if expected == {}:
                ...
                for dest_name in expected.keys():
                    if not self._get_nbr_data(intf, dest_name):
                        logger.debug(
                            "Skipping tunnel_route check for {} {} due to non-existing neighbor entry. "
                            .format(intf, dest_name))
                        continue
    ...
    def verify_tunnel_route(self, stand_alone=False):
        pytest_assert(
            wait_until(self.VERIFY_DB_TIMEOUT, 10, 0,
                       self.get_tunnel_route_mismatched_ports, stand_alone),
